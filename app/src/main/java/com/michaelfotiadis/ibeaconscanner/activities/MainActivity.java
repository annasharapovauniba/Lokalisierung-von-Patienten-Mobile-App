...
/**
 * Method updates list of beacons that were identified after each scan
 * */
...
*/
        mListAdapter = new ArrayAdapter<String>(com.michaelfotiadis.ibeaconscanner.activities.MainActivity.this,
                android.R.layout.simple_list_item_1);

        /*
       ArrayList<String> availableDevices = Singleton.getInstance().getDevicesAvailableAsStringList();

        if(items.size()!= 0) {
      //     availableDevices = Singleton.getInstance().getDevicesUpdatedAsStringList();

            for (int i = 0; i < items.size(); i++) {
                Log.d("TAG", "ItemsRemove");
                Log.d("TAG", "items.get(i)" + items.get(i));

                if (!availableDevices.contains(items.get(i))) {
        ...
        if(Singleton.getInstance().getAvailableDeviceListSize()==0){

        }else {
            for(int i = 0; i<availableDevices.size(); i++ ){
                if(items.contains(availableDevices.get(i))){

                }else{
                    items.add(availableDevices.get(i));
                }
            }
           // items = Singleton.getInstance().getDevicesAvailableAsStringList();
          //  Toast.makeText(this, items.get(0) +"\n"+ items.get(1), Toast.LENGTH_LONG).show();


            items.clear();
            items = Singleton.getInstance().getDetectedDevicesAsArrayList();
